...
def safe_mean_exclude_zero_nan(arr):
    arr = np.asarray(arr)
    arr = arr[np.isfinite(arr)]
    arr = arr[arr != 0]
    if arr.size > 0:
        return float(np.mean(arr))   # 转成原生 float
    else:
        return 0  # 或 float('nan')，根据需求
...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    geo_mean = safe_mean_exclude_zero_nan(network_info['geo_losses'])
    cla_mean = safe_mean_exclude_zero_nan(network_info['cla_losses'])
    l2_mean = safe_mean_exclude_zero_nan(network_info['l2_losses'])
    pre_mean = safe_mean_exclude_zero_nan(network_info['precisions'])
    recall_mean = safe_mean_exclude_zero_nan(network_info['recalls'])
    fscore_mean = safe_mean_exclude_zero_nan(network_info['f_scores'])

    return [float(ret_val), geo_mean, cla_mean, \
            l2_mean, pre_mean, \
            recall_mean, fscore_mean]


def test(data_loader, model, config):
    save_file_best = os.path.join(config.model_path, 'model_best1.pth')
